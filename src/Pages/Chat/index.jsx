...
import { GET_ALL_CHATS, INSERT_CHAT, GET_ROOM_CHAT } from '../../GraphQL/chat/queries';
...
const Chat = () => {
  ...
  const { data: dataRoom, loading: loadingRoomChat } = useSubscription(GET_ROOM_CHAT);
  ...
  const [insertChat, { loading: loadingInsertChat }] = useMutation(INSERT_CHAT, {
    onCompleted: (data) => {
      console.log(data);
    },
    onError: (error) => {
  ...
  useEffect(() => {
    if (dataRoom) {
      dataRoom.kanbo_chat_roomChat.filter((item) => {
        if ((item.user1 === room.user && item.user2 === room.friend) || (item.user1 === room.friend && item.user2 === room.user)) {
        ...
        <div className='px-4 py-4 mt-20'>
          <div className="basis-full h-screen container basis-full flex-row bg-primary-white rounded">
            <div className='p-6 container '>
              <div className='flex basis-full h-100 border-2 rounded' style={{ borderColor: "#D7D7D7" }}>
                <div className='basis-1/3 border-r-2' style={{ borderColor: "#D7D7D7" }}>
                    ...
                    <div className='flex flex-col'>
                      {
                        friend.map((item, index) => {
                          return (
                            <div onClick={() => { getClick(item.user) }} className='chat hover:bg-secondary-softblue flex p-4'>
                ...
                <div className='basis-2/3 relative'>
                  {
                    loadingRoomChat || loading ? <Loading></Loading> :
                      room.roomId !== "" ? (
                        <>

                          <div className='border-b-2' style={{ borderColor: "#D7D7D7" }}>
                            <div className='m-6 flex'>
                              <img src={pic1} alt="room" className='rounded-full mr-2 h-14' />
                              <div className='container flex-col self-center'>
                                <span><b>{room.friend}</b></span>
                      ...
                      <span className='text-center py-1 px-3 border-2 rounded-full'>
                        Fri, 18 Jun
                            ...
                            <div className='container p-4 flex-col'>
                              {
                                ...
                                roomChat.map((item, index) => (
                                  <div className={item.user === room.user ? "flex mt-8 justify-end" : "flex mt-8 "}>
                                    <div div className='max-w-xl p-4 bg-secondary-softblue rounded' >
                                      <span>
                                        <b>{item.user}</b>
                                      </span>
                                      <p>
                          ...
                          <form onSubmit={sendChat} className="container absolute bottom-0 left-0 right-0 bg-primary-blue py-3 px-4">
                            <input className='bg-secondary-softblue w-full py-1 px-4 rounded' type="text" placeholder='123' value={typing} onChange={(e) => { setTyping(e.target.value) }} />
                            <div className='ml-4' >
                              <button type='submit' className='py-1 px-4 rounded ' style={{ background: "white" }}>
                                Send
                              </button>
                            </div>
                          </form>
